            ...
            <div className="card mb-4 product-wap rounded-0">
                <div className="card rounded-0">
                    ...
                    <div className="card-img-overlay rounded-0 product-overlay d-flex align-items-center justify-content-center">
                        <a className="btn btn-success text-white mt-2" href={`/${roleLinkP}/${productID}`}>View</a>
                    </div>
                </div>
                    ...
                    <a href="/" className="text-decoration-none">{name}</a>
                    {
                        storefrontName ? (
                            <ul className="w-100 list-unstyled d-flex justify-content-between mb-0">
                                <li><a href={`/${roleLinkS}/${storefront}`} className="h3 text-decoration-none">{storefrontName}</a></li>
                            </ul>
                        ) : ''
                    }
                    <p className="text-center mb-0 h3">BDT {price}</p>
                </div>
            </div>
        </div>
    );
