...
export function OrderCard({
    ...
    switch (role) {
        case ROLES.CUSTOMER:
            ...
            badges.push(['Payment', `${Math.round(paymentBase + paymentVat)} BDT`]);
    ...
    return (
        <div className="card mb-2">
            <div className="card-header">
                <a href={`/productvo/${product}`} className="text-decoration-none text-secondary">{productName} (x{quantity}) - {new Date(orderTime).toLocaleString("en-US")}</a>
            </div>
