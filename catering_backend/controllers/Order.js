...
async function customerSubmitOrder(product, quantity, customer, destination) {
    if(quantity < 1) return CTRLRES(CTRLCODES.NOT_OK, 'Quantity must be greater than 0', 200, -1);

    const prod = await Product.findById(product);
    if(!prod) return CTRLRES(CTRLCODES.NOT_OK, 'Product not found', 200, -2);
    ...
    const order = new Order({
        ...
        storefront:     prod.storefront,
        storefrontName: prod.storefrontName,
        ...
        paymentBase:    basePrice,
        paymentVat:     vatAmount,
    });
    await order.save();
...
const post_customerSubmitOrder        = REQGATE({ func: customerSubmitOrder,        args: '*product, *quantity, customer:USER_ID, *destination' });
